...
export const changeUser = (
  ...
  createAction(
    ActionTypes.CHANGE_USER,
    CompleteUser.create(name, email, password)
  );
const saveSuccessful = (user: SavedUser) =>
...
export const save = () => (dispatch: Dispatch, getState: GetState) => {
  return matchUser<Promise<User>>(getState().user, {
    Complete: completeUser =>
      saveUser(completeUser).then(savedUser => {
...
const DEFAULT_STATE: UserReducerState = new IncompleteUser(
  new IncompleteNameInformation('', ''),
  new InitialEmail(),
  new EmptyPassword()
);
